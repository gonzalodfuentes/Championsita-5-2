...
public class Main extends ApplicationAdapter {
    private Menu menu; // Pantalla del men�
    private boolean inMenu = true; // Comenzamos en el men�
    ...
    public void render() {
        if (inMenu) {
            ...
            if (menu.shouldStartMatch()) {
                inMenu = false;
                menu.dispose(); // Liberar recursos del men�
    ...
    private void initializeMatch() {
        ...
        backgroundTexture = new Texture("stadium.png"); // Aseg�rate de tener esta textura

        // Crear las l�neas del campo (invisibles)
        createFieldLines();
    ...
    private void renderMatch() {
        ...
        camera.position.set(8, 4.5f, 0); // Centra la c�mara
        camera.update();
    ...
    // M�todo para crear la pelota
    private void createBall() {
        float ballRadius = 30f / 100; // 15 p�xeles de di�metro, ajustado con PPM
        ball = new Ball(world, 8, 5, ballRadius, true);
    }

    // M�todo para crear los jugadores
    ...
    // M�todo para crear las l�neas verdes del campo (suelo, paredes y techo)
